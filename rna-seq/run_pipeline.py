...
<table>
<tr><th>Name</th><th>Fastqc output</th></tr>
 ''')
newset = []
for d, f1, f2 in dataset:
    ...
    htmlmain.write('<tr><th>%s</th><td><a href="%s/fastqc_report.html" target="_blank">Read 1</a> <a href="%s/fastqc_report.html" target="_blank">Read 2</a></td></tr>'%(d, f1name, f2name))
dataset = newset
...
for d, f1, f2 in dataset:
    ...
    htmlmain.write('track type=bigWig db=%s name="%s Pos" visibility=2 color=255,0,0 bigDataUrl="%s/%s/rna-seq/%s"<br/>'%(genome, d, server['address'], server['home'], out1))
    htmlmain.write('track type=bigWig db=%s name="%s Neg" visibility=2 color=0,0,255 bigDataUrl="%s/%s/rna-seq/%s"<br/>'%(genome, d, server['address'], server['home'], out2))
    if not os.path.exists(workpath+out2):
        ## https://www.biostars.org/p/92935/
        bam = workpath+'%s.%s.tophat.bam'%(d, genome)
        load = float(subprocess.check_output("samtools view -F 4 "+bam+" | awk '{print $1}' | sort | uniq | wc -l", shell=True))
        os.system("samtools view "+bam+" -b -f 128 -F 16 | bedtools genomecov -bg -split -strand + -ibam stdin | awk '{print $1, $2, $3, "+str(1e8/load)+"*$4}' > tmp.bdg")
        os.system("samtools view "+bam+" -b -f 80 | bedtools genomecov -bg -split -strand + -ibam stdin | awk '{print $1, $2, $3, "+str(1e8/load)+"*$4}' >> tmp.bdg")
        os.system("bedGraphToBigWig tmp.bdg "+genome+".chrom.sizes "+workpath+out1)
        os.system("samtools view "+bam+" -b -f 144 | bedtools genomecov -bg -split -strand + -ibam stdin | awk '{print $1, $2, $3, -"+str(1e8/load)+"*$4}' | > tmp.bdg")
        os.system("samtools view "+bam+" -b -f 64 -F 16 | bedtools genomecov -bg -split -strand + -ibam stdin |awk '{print $1, $2, $3, -"+str(1e8/load)+"*$4}' >> tmp.bdg")
        os.system("bedGraphToBigWig tmp.bdg "+genome+".chrom.sizes "+workpath+out2)
