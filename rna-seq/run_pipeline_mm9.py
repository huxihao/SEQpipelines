...
parser.add_argument('--datapath', type=str, default='CJ160627/', help='Data path')
parser.add_argument('--workpath', type=str, default='work_nostrand/', help='Work path')
...
for d, f1, f2 in dataset:
    ...
    cmd = 'java -jar trimmomatic/trimmomatic-0.33.jar PE '+f1+' '+f2+' '+F1+' /dev/null '+F2+' /dev/null ILLUMINACLIP:trimmomatic/adapters/TruSeq3-PE-2.fa:2:30:10 LEADING:5 TRAILING:5 SLIDINGWINDOW:4:15 MINLEN:25'
...
for d, f1, f2 in dataset:
    ...
    cmd = 'tophat2 -G refseq_%s.gtf --transcriptome-index=transidx --library-type fr-secondstrand -p %s %s '%(genome, threads, genome)+f1+' '+f2+ '-o '+workpath+'tophat_out'
    if not os.path.exists(workpath+out):
        os.system(cmd)
        os.system('mv '+workpath+'tophat_out/accepted_hits.bam '+workpath+out)
        os.system('mv '+workpath+'tophat_out/junctions.bed '+workpath+out+'.junctions.bed')
        os.system('rm -rf '+workpath+'tophat_out')
...
#    out2 = '%s.%s.tophat_neg.bw'%(d, genome)
    htmlmain.write('track type=bigWig db=%s name="%s" visibility=2 bigDataUrl="%s/%s/rna-seq/%s"<br/>'%(genome, d, server['address'], server['home'], out1))
#    htmlmain.write('track type=bigWig db=%s name="%s Neg" visibility=2 bigDataUrl="%s/%s/rna-seq/%s"<br/>'%(genome, d, server['address'], server['home'], out2))
    if not os.path.exists(workpath+out1):
        os.system("bedtools genomecov -bg -split -ibam "+workpath+"%s.%s.tophat.bam > tmp.bdg"%(d, genome))
